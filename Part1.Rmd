  ...
  for (depth in max_depth) {
    for (mtry in mtry_values) {
      for (i in seq_along(folds)) {
        ...
        # Store results in the preallocated dataframe
...
```{r violin plot - random forest}
box_plots <- list()
for (number_of_trees in num_trees) {
  # Filter the data for the current number of trees
  data_subset <- results_df[results_df$num_trees == number_of_trees, ]
  
  # Create the box plot for the current number of trees
  p <- ggplot(data_subset, aes(x = factor(paste("Depth", depth, "Mtry", mtry)), y = MSE)) +
    geom_boxplot(fill = "lightblue", color = "darkblue") +  # Create box plots
    geom_jitter(color = "darkred", size = 1.0, width = 0.2, alpha = 0.6) +  # Overlay points
  ...
  # Store the plot in a list, using the number of trees as the key
